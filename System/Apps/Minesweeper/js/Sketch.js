...
function createCanvas(parent, id = "board", w = 258, h = 258) {
...
function removeChildren(el) {
  const p = typeof el === "string" ? document.getElementById(el) : el;
...
function addAlert(type) {
  ...
  const content = document.getElementById("customAlert-content");
  removeChildren(content);

  type.text.forEach(t => {
    if (t === "br") content.appendChild(document.createElement("br"));
    else content.appendChild(document.createTextNode(t));
  });
...
function removeAlert() {
  ...
  setup(document.getElementById("board"));
}
...
function addStartListener(canvas) {
...
function addExplodeListener(canvas) {
  canvas.addEventListener("explode", evt => {
    const explosion = new Explosion(evt.detail.x, evt.detail.y);
    const anim = setInterval(() => {
      board.draw(canvas);
      explosion.update(canvas);
    }, 1000 / 15);

    setTimeout(() => clearInterval(anim), 900);
  });
}

function addStatusListener(canvas) {
  canvas.addEventListener(WIN.name, () => addAlert(WIN));
  canvas.addEventListener(LOSE.name, () => {
    setTimeout(() => addAlert(LOSE), 500);
  });
}
...
function setup(canvas) {
  clearInterval(timerInterval);
  timer.reset();
  updateTextNode("timer", "00:00");

  // ✅ IMPORTANT: board size fix
  board = new Board(8, 8); // 8x8 = 64 tiles (classic)

  board.draw(canvas);
  updateTextNode("mines", board.mineNumber);
  addStartListener(canvas);
}
